...
const SURVEY = {
  ...
  options: [
    { id: 'a', title: 'AI Chat Assistant', description: 'Add a conversational AI assistant that can answer questions about your data, generate reports, and suggest insights directly from the dashboard.', emoji: '🤖' },
    { id: 'b', title: 'Custom Alert Rules', description: 'Set threshold-based alerts (email/Slack) when metrics spike or drop. Define conditions like "notify me when query errors > 50/hr".', emoji: '🔔' },
    { id: 'c', title: 'Team Collaboration', description: 'Invite teammates, assign roles, share dashboards, and leave comments on charts and reports.', emoji: '👥' },
    { id: 'd', title: 'Data Export & Scheduling', description: 'Export any chart or report as PDF/CSV/Excel on demand, or set up scheduled email digests sent automatically.', emoji: '📤' },
  ],
}
...
export default function SurveyPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (voted) { setHasVoted(true); setMyVote(voted) }
    } catch {}
    setMounted(true)
  }, [])
  ...
  function castVote(optionId) {
    ...
    setVotes(newVotes); setHasVoted(true); setMyVote(optionId)
    try {
  ...
  const winnerId = total > 0 ? Object.entries(votes).sort((a, b) => b[1] - a[1])[0][0] : null
      ...
      <div className="survey-layout">
        <div>
          <div className="card" style={{ marginBottom: 16 }}>
            <p style={{ fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.7 }}>
              ...
              Vote below, and the winner gets built and deployed this Friday.
            </p>
          </div>
            ...
            return (
              <div key={opt.id}
                ...
                <div className="vote-bar-wrap">
                  <div className={`vote-bar ${isWinner ? 'winner' : ''}`}
                    style={{ width: mounted && hasVoted ? `${pct}%` : '0%' }} />
                </div>
                <div className="vote-count">
                  {mounted && hasVoted ? `${votes[opt.id]} votes · ${pct}%` : 'Tap to vote'}
                </div>
              </div>
            )
          })}
        ...
        <div>
          <div className="card" style={{ marginBottom: 16 }}>
            ...
            {[['Mon–Thu', "Vote on this week's 4 options"], ['Friday', 'Winner gets built & deployed'], ['Friday+', 'New survey goes live'], ['Backlog', 'Runners-up saved for later']].map(([stage, desc]) => (
          ...
          <div className="card">
            ...
            <div style={{ marginTop: 8 }}>
              {[['Total Votes', mounted ? total.toString() : '—'], ['Options', '4'], ['Closes', 'Friday'], ['Backlog Items', '0']].map(([k, v]) => (
